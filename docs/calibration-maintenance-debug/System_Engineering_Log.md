...
# ISSUE-009 – Refactor Nutrient Pump Execution Architecture and Secure ESP-Override Controls
...
Create one authoritative nutrient-pump execution architecture that separates
dose calculation from physical pump execution while preserving all required
Fill, Maintenance, Manual Dose, and ESP-Override functionality.

The refactor must eliminate unsafe or ambiguous operator paths that can energize
a nutrient pump without:

- a defined dose quantity,
- a calculated maximum runtime,
- automatic pump shutoff,
- dose accounting,
- controller-state visibility,
- and event logging.

Direct nutrient-pump relay switches must not remain exposed as normal operator
controls. Any retained direct relay control must be restricted to an explicitly
enabled service or maintenance mode and clearly identified as bypassing normal
dose execution safeguards.

This refactor will:

- reduce duplicated nutrient-pump execution code,
- provide one controlled execution path for measured doses,
- prevent accidental indefinite pump operation,
- maintain accurate nutrient totals and history,
- improve controller-state reporting,
- simplify future enhancements,
- and reduce inconsistencies between Fill, Maintenance, Manual Dose, and
  ESP-Override operation.

## Safety Requirement

A normal dashboard operator must not be able to directly latch Nutrient Pump A
or Nutrient Pump B ON.

All normal nutrient additions must be initiated as a requested quantity in mL
and executed by a bounded ESPHome dosing script.

The dosing script must:

1. Validate the requested quantity.
2. Convert the quantity to pump runtime.
3. Apply a configured maximum dose and maximum runtime.
4. Energize the selected pump.
5. Automatically de-energize the pump when the runtime expires.
6. Update the measured-dose and runtime entities.
7. Expose execution state to the controller state machine.
8. Produce sufficient information for event logging.

Any direct relay control retained for diagnostics must:

- be hidden from the normal operating dashboard,
- require an explicit service-mode enable,
- use momentary or time-limited execution rather than an indefinite latch,
...
Pump B remained energized for approximately 12 seconds until manually turned
off.

The raw relay path:

- did not use the requested 10 mL quantity,
- did not automatically stop,
- did not update the expected dose accounting,
- and could have caused a substantially larger overdose if not noticed.

A compensating second 10 mL Nutrient A dose was then performed because the
actual Nutrient B quantity was uncertain.

This incident confirms that raw nutrient-pump switches must not be presented as
...
# ISSUE-011 Voltge Measurements not working for Auto Dose. Add 2nd DFRobot Gravity TDS Meter
...
Future analog channels remain available for additional sensors.

### Engineering Measurement Model

Legacy "TDS" field names were retired from the engineering data model.

Maintenance events now store:
...
EC reference records now store synchronized snapshots of:
...
The handheld EC meter remains the engineering reference standard.
...
Legacy probe_voltage and TDS voltage fields were removed in favor of explicit
A0/A1 engineering measurements.

### Node-RED

History Browser and Cycle Manager were updated to use the new engineering
measurement model.

Operator-facing history cards intentionally display filtered voltages while raw
measurements remain stored for engineering analysis and future calibration.

### Current Status

The engineering measurement migration has been verified through:
...
Remaining work is limited to EC calibration and future control-model
improvements.
